...
- **Never write `[REDACTED]` back as if it were a real value.** If you read a resource's env with the sentinel in place and then need to update that same resource, don't round-trip the sentinel string into the write — that overwrites the actual secret with the literal placeholder text. For a Swarm stack, use `updateSwarmStack`'s diff parameters (`env_set`/`env_unset`/`env_replace`) so unmentioned variables are left untouched server-side; you never need to know their current value to preserve them. For other resources (services, containers, K8s objects) where no diff-aware tool exists, don't attempt a read-modify-write of the env block at all — ask the user for the real value, or have them make the change directly. The server also rejects any write whose body contains the sentinel while redaction is active, so this fails loudly rather than silently corrupting data — but it's cheaper to avoid the round trip than to hit the error.
